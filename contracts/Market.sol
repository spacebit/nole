...
contract Market is NilBase {
    mapping(CurrencyId nftId => Order) private s_orders;
    mapping(address => mapping(CurrencyId currencyId => uint256 balance)) private s_virtualBalance;

    event Put(address indexed owner, uint256 price);
    event Withdraw(address indexed owner, CurrencyId indexed tokenId, uint256 amount);
    event Deposit(address indexed owner, CurrencyId indexed tokenId, uint256 amount);

    constructor() {}

    receive() external payable {}

    function put(CurrencyId _nftId, CurrencyId _currencyId, uint256 _price) public onlyInternal {
    ...
    function buy(CurrencyId _nftId) external payable {
        ...
        if (order.currencyId == CurrencyId.wrap(address(0))) {
    ...
    function handleDepositNFT(bool _success, bytes memory, bytes memory _context) public onlyResponse {
        if (!_success) {
            ...
            (CurrencyId nft, address buyer) = abi.decode(_context, (CurrencyId, address));
    ...
    function getBalance(address _owner, CurrencyId _token) external view returns (uint256) {
        return s_virtualBalance[_owner][_token];
    }

    function getOrder(CurrencyId _nftId) external view returns (Order memory) {
        return s_orders[_nftId];
    }

    function getPendingBuyer(CurrencyId _nftId) external view returns (address) {
    ...
    function _deposit(address _owner, CurrencyId _tokenId, uint256 _amount) private {
